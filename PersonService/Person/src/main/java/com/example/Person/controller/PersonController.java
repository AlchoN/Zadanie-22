...
public class PersonController {
    ...
    public ResponseEntity<Object> getWeather(@PathVariable int id) {
        ...
        if (personOptional.isPresent()) {
            ...
            String[] coordinates = person.getLocation().split(",");
            if (coordinates.length != 2) {
                return new ResponseEntity<>("Неверный формат локации", HttpStatus.BAD_REQUEST);
            }
            String lat = coordinates[0].trim();
            String lon = coordinates[1].trim();
            String url = String.format("http://localhost:8082/weather?lat=%s&lon=%s", lat, lon);
